...
4. **NowPlaying data is never persisted, never in a log message at any level, and never leaves the device — except title/artist/album/duration to `lrclib.net` for lyrics, and only when the user has opted in.** Title/artist/album art is the user's listening history; a single `Log.d` of it writes that history to logcat where other tooling can read it. Full rule: X3 in `references/security.md`.
